    ...
    <div className="fixed inset-0 z-[140] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-nova-surface border border-nova-border rounded-xl shadow-2xl w-[800px] max-h-[80vh] flex flex-col animate-fade-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-nova-border">
          <h2 className="text-lg font-semibold text-nova-text-primary">Model Comparison</h2>
        ...
        <div className="p-4 border-b border-nova-border">
          <div className="flex items-center gap-3">
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search models..." className="flex-1 px-3 py-2 bg-nova-input-bg border border-nova-border rounded-lg text-sm text-nova-text-primary outline-none focus:border-nova-accent/50" />
            <span className="text-xs text-nova-text-muted">Select up to 4 models to compare</span>
          </div>
          <div className="flex flex-wrap gap-2 mt-3">
            {filteredModels.slice(0, 30).map((m) => (
              <button key={m.id} onClick={() => toggleModel(m.id)} className={`px-2 py-1 text-xs rounded-lg border transition-colors ${selectedModels.includes(m.id) ? 'border-nova-accent bg-nova-accent/10 text-nova-accent' : 'border-nova-border text-nova-text-secondary hover:border-nova-accent/50'}`}>
                {m.id.split('/').pop() || m.id}
                {m.isFree && <span className="ml-1 text-green-400">(Free)</span>}
              </button>
            ))}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-nova-border">
                  <th className="text-left py-2 text-xs text-nova-text-muted font-medium">Feature</th>
              ...
              <tbody>
                <tr className="border-b border-nova-border/50">
                  <td className="py-2 text-xs text-nova-text-secondary">Context Window</td>
                  {selectedModels.map((id) => {
                    const meta = getMetadata(id)
                    return <td key={id} className="text-center py-2 text-xs text-nova-text-primary">{meta ? `${(meta.contextWindow / 1000).toFixed(0)}K` : 'N/A'}</td>
                  })}
                </tr>
                <tr className="border-b border-nova-border/50">
                  <td className="py-2 text-xs text-nova-text-secondary">Pricing</td>
                  {selectedModels.map((id) => {
                    const meta = getMetadata(id)
                    return <td key={id} className="text-center py-2 text-xs text-nova-text-primary">{meta?.pricing || 'N/A'}</td>
                  })}
                </tr>
                <tr className="border-b border-nova-border/50">
                  <td className="py-2 text-xs text-nova-text-secondary">Vision Support</td>
                  {selectedModels.map((id) => {
                    const meta = getMetadata(id)
                    return <td key={id} className="text-center py-2">{meta?.vision ? <span className="text-green-400">Yes</span> : <span className="text-nova-text-muted">No</span>}</td>
                  })}
                </tr>
                <tr className="border-b border-nova-border/50">
                  <td className="py-2 text-xs text-nova-text-secondary">Function Calling</td>
                  {selectedModels.map((id) => {
                    const meta = getMetadata(id)
                    return <td key={id} className="text-center py-2">{meta?.functionCall ? <span className="text-green-400">Yes</span> : <span className="text-nova-text-muted">No</span>}</td>
                  })}
                </tr>
                <tr>
                  <td className="py-2 text-xs text-nova-text-secondary">Free</td>
                  {selectedModels.map((id) => {
                    const m = models.find((m) => m.id === id)
                    return <td key={id} className="text-center py-2">{m?.isFree ? <span className="text-green-400">Yes</span> : <span className="text-nova-text-muted">No</span>}</td>
                  })}
                </tr>
              </tbody>
            </table>
